...
export async function getNpsResponses(): Promise<NpsResponseRow[]> {
  ...
  const npsQuestionIds = new Set(
    ((questions as { id: string; question_text: string }[]) ?? [])
      .filter((q) => normalizeQuestionText(q.question_text).includes(NPS_QUESTION_ANCHOR))
      .map((q) => q.id),
  )
...
>>>>>>> 08f618616d90ffb4f29aac228b42fc75f18aa15e
 */
